...
public class ReplaceDeleteAction
    extends BaseSnapshotUpdateAction<ReplaceDeleteAction, DeleteRewriteActionResult> {
  ...
  public DeleteRewriteActionResult execute() {
    ...
    try {
      fileScanTasks = table.newScan().caseSensitive(caseSensitive).ignoreResiduals().planFiles();
    } finally {
      try {
    ...
    CloseableIterable<FileScanTask> tasksWithEqDelete =
    ...
    Map<StructLikeWrapper, Collection<FileScanTask>> groupedTasks =
        groupTasksByPartition(tasksWithEqDelete.iterator());

    // Split and combine tasks under each partition
    // TODO: can we split task?
    List<Pair<StructLike, CombinedScanTask>> combinedScanTasks =
                ...
                pair ->
                    StreamSupport.stream(
                        CloseableIterable.transform(
                                pair.second(), task -> Pair.of(pair.first(), task))
                            .spliterator(),
                        false))
            .collect(Collectors.toList());
  ...
  private void rewriteDeletes(List<DeleteFile> eqDeletes, List<DeleteFile> posDeletes) {
    Preconditions.checkArgument(
        eqDeletes.stream().allMatch(f -> f.content().equals(FileContent.EQUALITY_DELETES)),
        "The deletes to be converted should be equality deletes");
    Preconditions.checkArgument(
        posDeletes.stream().allMatch(f -> f.content().equals(FileContent.POSITION_DELETES)),
