...
public class TestRewriteDeletesAction extends SparkTestBase {
  ...
  public void testRewriteDeletesWithNoEqDeletes() {
    ...
    DeleteRewriteActionResult result =
        SparkActions.get(spark).replaceEqDeleteToPosDelete(table).execute();
  ...
  public void testRewriteDeletesInPartitionedTable() throws IOException {
    PartitionSpec spec = PartitionSpec.builderFor(SCHEMA).identity("c1").truncate("c3", 2).build();
    Map<String, String> options = Maps.newHashMap();
    BaseTable table = (BaseTable) TABLES.create(SCHEMA, spec, options, tableLocation);
    TableOperations ops = table.operations();
    ops.commit(ops.current(), ops.current().upgradeToFormatVersion(2));
    ...
    PartitionKey key =
        createPartitionKey(
            ...
            GenericRecord.create(table.schema())
                .copy(ImmutableMap.of("c1", 1, "c2", "ignore", "c3", "AAAA")));
    ...
    try (EqualityDeleteWriter<Record> closeableWriter = eqDeleteWriter) {
      closeableWriter.write(
          GenericRecord.create(eqDeleteRowSchema).copy(ImmutableMap.of("c2", "AAAAAAAA")));
    }
  ...
  public void testRewriteDeletesWithDeletes() throws IOException {
    ...
    Record partitionRecord =
        GenericRecord.create(table.schema().select("c1")).copy(ImmutableMap.of("c1", 1));
    ...
    PositionDeleteWriter<Record> posDeleteWriter =
        appenderFactory.newPosDeleteWriter(
            file, FileFormat.PARQUET, createPartitionKey(table, partitionRecord));
    posDeleteWriter.write((PositionDelete<Record>) partitionRecord);
    ...
    DeleteRewriteActionResult result =
        SparkActions.get(spark).replaceEqDeleteToPosDelete(table).execute();
    Assert.assertEquals(
        "Should contain one equality delete to delete", 1, result.deletedFiles().size());
