...
interface HiveOperationsBase<T> {
  ...
  String metadataKeyValue(T metadata, String key, String defaultValue);

  BaseMetastoreTableOperations.CommitStatus validateNewLocationAndReturnCommitStatus(
      T metadata, String newMetadataLocation);

  Set<String> obsoleteProps(T base, T metadata);
  ...
  boolean hiveEngineEnabled(T metadata);

  boolean hiveLockEnabled(T metadata);
  ...
  StorageDescriptor storageDescriptor(T metadata, boolean hiveEngineEnabled);
  ...
  default HiveLock lockObject(T metadata, Configuration conf, String catalogName) {
