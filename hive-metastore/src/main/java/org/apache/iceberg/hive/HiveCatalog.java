...
public class HiveCatalog extends BaseMetastoreViewCatalog
  ...
  public List<TableIdentifier> listTables(Namespace namespace) {
    ...
    try {
      ...
      if (listAllTables) {
        tableIdentifiers = listAllTables(namespace);
      } else {
        tableIdentifiers =
            listTablesByType(
                namespace,
      ...
      LOG.debug(
          "Listing of namespace: {} resulted in the following: {}", namespace, tableIdentifiers);
  ...
  public boolean dropView(TableIdentifier identifier) {
    ...
    try {
      ...
      HiveViewOperations.validateTableIsIcebergView(table, fullTableName(name, identifier));
      clients.run(
          client -> {
  ...
  private List<TableIdentifier> listTablesByType(
    ...
    // Each TableData is a big object.Retrieving it with a big number can put pressure on memory.
  ...
  public void renameView(TableIdentifier from, TableIdentifier originalTo) {
    ...
    try {
      ...
      HiveViewOperations.validateTableIsIcebergView(fromView, fullTableName(name, from));
      Table view = null;

      try {
        view = clients.run(client -> client.getTable(toDatabase, to.name()));
      } catch (NoSuchObjectException nte) {
        LOG.trace("View not found {}.{}", toDatabase, to.name(), nte);
      }
      ...
      throw new org.apache.iceberg.exceptions.AlreadyExistsException(e.getMessage());
    } catch (TException e) {
      throw new RuntimeException("Failed to rename " + from + " to " + to, e);
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
